"""
    ...
    Project Group: 3
    Topic: Polar Codes
    Sub-topic: Polar Code Successive Cancellation Decoding, Bit Error Rate an
"""
...
def minsum(L1, L2):
    ...
    min_sum = [0] * n
    for i in range(n):
        min_sum[i] = (1 - 2 * (L1[i] < 0)) * (1 - 2 * (L2[i] < 0)) * min(abs(L1[i]), abs(L2[i]))
    return min_sum
...
monte_carlo(N=8, K=4)
monte_carlo(N=16, K=8)
monte_carlo(N=32, K=16)
monte_carlo(N=64, K=32)
monte_carlo(N=128, K=64)

monte_carlo(N=256, K=128)
monte_carlo(N=512, K=256)
# monte_carlo(N=1024, K=512)                          
