...
print()
Reliability_Sequence = [0, 1, 2, 4, 8, 16, 32, 3, 5, 64, 9, 6, 17, 10, 18, 128, 12, 33, 65, 20, 256, 34, 24, 36, 7, 129, 66, 
...
received_sequqnce = AWGN(bpsk_modulated_sequence)
Likelihood = received_sequqnce
print(Likelihood)

# Time Complexity: NLogN
def decode(L, node, depth):

    # We are 1at leaf node
    if depth == 0:
    ...
    u_cap_combined_left, u_cap_left = decode(left_child_beliefs, 2*node, depth-1)

    # Step R: Decode for the right child
    right_child_beliefs = g(r1, r2, u_cap_combined_left)
    u_cap_combined_right, u_cap_right = decode(right_child_beliefs, 2*node + 1, depth-1)
...
enc_msg, decoded_message = np.array(decode(Likelihood, 0, n))
...
if isEqual(u_sequence, decoded_message):
