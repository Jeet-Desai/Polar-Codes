...
def polar_code(N, K, SCL_N):
    ...
    plt.figure(3)
    plt.plot(EbNodB_values, np.array(BLER), label = f"N={N}, K={K}, M={SCL_N} FER") 
    plt.ylim(1e-4, 1)
    plt.yscale("log")
    plt.xlabel("Eb/No (dB)")  
    plt.ylabel("FER")  
    plt.title("FER v/s SNR For SCL Decoder")
    plt.legend(fontsize=10)
    plt.grid(which="both", axis="both")
...
# polar_code(N=16, K=8, SCL_N=4)
# polar_code(N=32, K=16, SCL_N=4)
polar_code(N=64, K=32, SCL_N=4)
# polar_code(N=512, K=80, SCL_N=4)
plt.show()
